...
void Image_Encoder::forward(
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        hidden_totals[hidden_cell_index] = 0;
    }
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        count_all += (iter_upper_bound.x - iter_lower_bound.x + 1) * (iter_upper_bound.y - iter_lower_bound.y + 1) * vld.size.z;
            ...
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int vc = 0; vc < vld.size.z; vc++) {
                    ...
                    for (int hc = 0; hc < hidden_size.z; hc++) {
                        ...
                        hidden_sums[hidden_cell_index] += min(input, vl.weights0[wi]) + min(static_cast<Byte>(255 - input), vl.weights1[wi]);
                        hidden_totals[hidden_cell_index] += vl.weights0[wi] + vl.weights1[wi];
                    }
                }
            }
    }
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        int total = hidden_totals[hidden_cell_index];

        float sumf = sum * byte_inv;

        float match = sumf / count_all;

        float activation = sumf / (params.choice + total * byte_inv);
            ...
            for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
                for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        vl.weights1[wi] = static_cast<Byte>(255 - input);
                    }
                }
        }
...
long Image_Encoder::size() const {
    long size = sizeof(Int3) + sizeof(Params) + hidden_cis.size() * sizeof(int) + hidden_committed_flags.size() * sizeof(Byte) + sizeof(int);
